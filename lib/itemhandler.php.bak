...
function display_item_fightnav($args) {
	...
	$sql = "SELECT $item.itemid, $item.link,
					...
					GROUP BY $item.itemid, $inventory.invid, $item.link";	
	$result = db_query($sql);
...
function get_item_by_name($itemname) {
	$sql = "SELECT itemid FROM ".db_prefix("item")." WHERE name = '$itemname' LIMIT 1";
	$result = db_query($sql);
	$item = db_fetch_assoc($result);
	if (is_array($item)==false) return false;
	return get_item_by_id($item['itemid']);
}

function get_item_by_id($itemid) {
	if (!isset($itemid)) return false;
	static $allitems = array();
...
function get_random_item($class = false, $repeat=true) {
	$chance = e_rand(0,100);
	if ($class === false) {
	...
	if (db_num_rows($result) == 1) $item = db_fetch_assoc($result);
		...
		//2. Versuch
		if ($repeat==true) {
			$item=get_random_item($class,false);
		} else {
			$item = false;
		}
	}
...
debug($sql);
	...
	if ($maxcount > 0) {
		debug("In theory only $maxitems_count could be added (totalcount)");
	} else {
		debug("There is no restriction on quantity active.");
	}
	if ($maxweight > 0) {
		debug("In theory only $maxitems_weight could be added (totalweight)");
	}else {
		debug("There is no restriction on weight active.");
	}

	if ($maxcount > 0 && ($maxweight != 0 && $item_raw['weight'] > 0)) {
...
function get_inventory($user=0, $showhide=false, $class=0, $singleonly=false, $simplegroup=false) {
	global $session;
	...
	if ($singleonly === false) {
		...
		$sql = "SELECT $item.*, $inventory.invid AS invid, COUNT($inventory.itemid) AS quantity,
					...
					WHERE inventory.userid = $user
						AND $item.hide = $showhide $classwhere
					GROUP BY $inventory.itemid, $inventory.sellvaluegold, $inventory.sellvaluegems
					ORDER BY
						$item.class ASC,
						$item.name ASC";
		}
...
function get_itemnames_by_class($class) {
	if(func_num_args() > 1 || is_array($class)) {
		if(!is_array($class)) {
			$class = func_get_args();
		}
		$result = db_query("SELECT itemid, name FROM item WHERE class IN('".join("','", $class)."')");
	} else {
		$result = db_query("SELECT itemid, name FROM item WHERE class='$class'");
	}
	$ids = array();
	while($id = db_fetch_assoc($result)) {
		$ids[] = $id;
...
function uncharge_item($itemid, $user=false, $invid=false, $charges=1) {
	global $session;
	if (!is_int($itemid)) {
		$itemid = get_item_by_name($itemid);
		$itemid = $itemid['itemid'];
	...
	if ($invid !== false)
		...
		$invsql = "";
	$inventory = db_prefix("inventory");
	$sql = "UPDATE $inventory SET charges = charges - $charges WHERE itemid = $itemid AND userid = $user AND charges >= 1 $invsql LIMIT 1";
	$result = db_query($sql);
...
function recharge_item($itemid, $user=false, $invid=false, $charges=1) {
	global $session;
	if (!is_int($itemid)) {
		$itemid = get_item_by_name($itemid);
		$itemid = $itemid['itemid'];
	...
	$inventory = db_prefix("inventory");
	$sql = "UPDATE $inventory SET charges = charges + $charges WHERE itemid = $itemid AND userid = $user $invsql LIMIT 1";
	$result = db_query($sql);
	if (db_affected_rows($result) == 0) {
...
function remove_items_by_class($class) {
	...
	foreach($ids as $id) {
		remove_item((int)$id['itemid'], check_qty((int)$id['itemid']));
	}
}
...
function shopnav($return, $class, $sell=false, $user=false, $sellall=false, $showdescription=true) {
	...
	if (db_num_rows($result) > 0) {
		...
		while($row = db_fetch_assoc($result)) {
			$row = modulehook("shopnav", array('item'=>$row, 'sell'=>$sell));
			if (isset($row['disable']) && $row['disable'] === true) {
				continue;
			}
			$item = $row['item'];
			if ($sell === false) {
				if ($session['user']['dragonkills'] > $item['dragonkills'] || ($session['user']['level'] >= $item['level'] && $session['user']['dragonkills'] == $item['dragonkills'])) {
					if ($session['user']['gold'] >= $item['gold'] && $session['user']['gems'] >= $item['gems']) {
						addnav(array("%s`n`^%s`0 Gold, `%%%s`0 Gems", translate_inline($item['name']), $item['gold'], $item['gems']), $return."id=".$item['itemid']);
					} else {
						addnav(array("%s`n`^%s`0 Gold, `%%%s`0 Gems", translate_inline($item['name']), $item['gold'], $item['gems']), "");
					}
					if ($showdescription == true) {
						output("`#%s ", translate_inline($item['name']));
						$qty = check_qty((int)$item['itemid']);
						if ($qty > 0) {
							output("`7- You own `^%s pieces", $qty);
						}
						$description = translate_inline($item['description']);
						output("`n`7%s`n`n", $description);
					}
				}
				
			} else {
				addnav(array("%s`n`^%s`0 Gold, `%%%s`0 Gems`n(`2%s Stück`0)", translate_inline($item['name']), $item['sellvaluegold'], $item['sellvaluegems'], $item['quantity']), $return."id=".$item['itemid']);
			}
		}
		tlschema();
	}
