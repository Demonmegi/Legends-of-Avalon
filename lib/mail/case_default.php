...
if ($detail=="" && $displaymode!=2) {
	...
	if ($db_num_rows>0){
		...
		rawoutput("<td><a href='mail.php?ordering=total&direction=" . (($ordering=='total'&& $direction=='ASC')?'DESC':'ASC') . "'>" . translate_inline('total') . "</a></td>");
		...
		rawoutput("<td><a href='mail.php?ordering=sent&direction=" . (($ordering=='sent'&& $direction=='ASC')?'DESC':'ASC') . "'>" . translate_inline('newest') . "</a></td>");
		rawoutput("</tr>");
		...
		if ($totalmails == 1) {
			...
			output("`n`n`iYou currently have %s messages in your inbox.`nYou will no longer be able to receive messages from players if you have more than %s unread messages in your inbox.  `nMessages are automatically deleted (read or unread) after %s days.",$totalmails,getsetting('inboxlimit',50),getsetting("oldmail",14));
		}
	}else{
