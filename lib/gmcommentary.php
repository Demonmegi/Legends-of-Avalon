		...
		if ($section || $talkline || $comment) {
			...
			if ($tcom == "" || $tcom == ":" || $tcom == "::" || $tcom == "/me")
				...
				if (rawurldecode(httpget('section'))!=$section) {
					...
					gminjectcommentary($section, $talkline, $comment, $schema);
				}
			}
...
function gminjectsystemcomment($section,$comment) {
	...
	gminjectrawcomment($section,0,$comment);
}

function gminjectrawcomment($section, $author, $comment)
{
...
function gminjectcommentary($section, $talkline, $comment, $schema=false) {
	...
	if ($comment !="") {
		...
		if (substr($commentary,0,5)=="/game" && ($session['user']['superuser']&SU_IS_GAMEMASTER)==SU_IS_GAMEMASTER) {
			...
			gminjectsystemcomment($section,$commentary);
		} else {
			...
			if ($row['comment']!=stripslashes($commentary) ||
				...
				gminjectrawcomment($section, $session['user']['acctid'],
						$commentary);
...
function gmcommentdisplay($intro, $section, $message="Interject your own commentary?",$limit=10,$talkline="says",$schema=false,$overview=false) {
	// Let's add a hook for modules to block commentary sections
	$args = modulehook("blockcommentarea", array("section"=>$section));
	if (isset($args['block']) && ($args['block'] == "yes"))
		return;

	if (!isset($overview)) $overview=false;

	if ($intro) output($intro);
	gmviewcommentary($section, $message, $limit, $talkline, $schema,$overview);
}

function gmviewcommentary($section,$message="Interject your own commentary?",$limit=10,$talkline="says",$schema=false,$overview) {
	...
	if ($cid == 0) {
		...
		$sql = "SELECT " . db_prefix("commentary") . ".*, " .
			...
			db_prefix("accounts") .
			".clanid WHERE ( ".db_prefix("accounts") . ".locked=0 OR ".db_prefix("accounts") .".locked is null ) AND commentid > '$cid' ";
			"( ".db_prefix("accounts") . ".locked=0 OR ".db_prefix("accounts") .".locked is null ) ";
	...
	$sections = gmcommentarylocs();
	$needclose = 0;

	foreach ($outputcomments as $sec=>$v) {
		if ($sec!="x") {
			if($needclose) modulehook("}collapse");
			output_notl("`n<hr><a href='runmodule.php?module=gmmodview&area=%s'>`b`^%s`0`b</a>`n",
				$sec, isset($sections[$sec]) ? $sections[$sec] : "($sec)", true);
			addnav("", "runmodule.php?module=gmmodview&area=$sec");
	...
	if ($session['user']['loggedin'] && $overview==false) {
...
function gmtalkform($section,$talkline,$limit=10,$schema=false){
	...
	previewfield("insertcommentary", $session['user']['name'], $talkline, true, array("size"=>"60", "maxlength"=>200-$tll));
