<?php
	$moddb = db_prefix("moderatedcomments");
	...
	$sql = "SELECT name, $moddb.* FROM $moddb LEFT JOIN " . db_prefix("accounts") . " ON acctid=moderator WHERE $where ORDER BY moddate DESC LIMIT $limit";
	$result = db_query($sql);
	$i=0;
