<?php
// Include necessary files and initialize $session (assuming common.php contains session initialization)
...
if ($loc == "") {
    ...
    $i = 0;
    $row = array();
    while ($row[$i] = db_fetch_assoc($result)) $i++;
    if ($rownumber > 0) {
        addnav("Other Houses");
        for ($i = 0; $i < $rownumber; $i++) {
            addnav(stripslashes($row[$i]['name']), "runmodule.php?module=house&lo=house&id=" . $row[$i]['houseid']);
        }
    }
}
...
switch ($loc) {
    ...
    default:
        ...
        if (db_num_rows($result) > 0) {
            ...
            output("`3After walking for a while you decide quite assuredly that there's simply nothing here!");
