...
function claneditor_getmoduleinfo(){
	$info = array(
		"name"=>"Clan Editor",
		"version"=>"1.2",
		"author"=>"<a href='http://cortalux.tczhost.net' class='colLtGreen'>CortalUX</a>, `7minor changes by `2R`@o`ghe`2n `Qvon `2Fa`@lk`genbr`@uch`0",
...
function claneditor_run(){
	...
	if ($op==""||$op=="list"){
		...
		$cdb=db_prefix("clans");
		$adb=db_prefix("accounts");
		$sql = "SELECT $cdb.clanid, $cdb.clanshort, $cdb.clanname,count($adb.acctid) AS c FROM $cdb LEFT JOIN $adb ON $cdb.clanid=$adb.clanid AND clanrank>".CLAN_APPLICANT." GROUP BY $cdb.clanid ORDER BY c DESC";
		$result = db_query($sql);
		if (db_num_rows($result)>0){
			...
			rawoutput('<table width=400 cellspacing="0" cellpadding="2" align="left">');
			output_notl("<tr class='trhead'><td>%s</td><td>%s</td><td>%s</td></tr>",translate_inline("`b`&Clan Tag`b"),translate_inline("`b`&Name of Clan`b"),translate_inline("`&`iNumber of Members`i"),true);
			for ($i=0;$i<db_num_rows($result);$i++){
				...
				if ($row['c']==0){
					...
					output_notl("&lt;%s&gt;</td><td>&#149; <a href='runmodule.php?module=claneditor&op=mview&dt=%s'>%s</a></td><td>%s`n",
							$row['clanshort'],
							$row['clanid'],
		...
		if ($apply==1){
			...
			$e2 = translate_inline("`%Karissa`7 looks over your form but informs you that you must have at least 5 and no more than 255 characters in your clan's name (and they must consist only of letters, spaces, apostrophes, or dashes), then hands you a blank form.");
			...
			if ($ocs==""&&$ocn==""&&!httppostisset('clanname')&&!httppostisset('clanshort')) {
				...
				output_notl($e1);
				clanform();
			}elseif (strlen($clanname)<5 || strlen($clanname)>255){
				output_notl($e2);
				clanform();
		...
		if ($op=="mview"){
			...
			if (httpget("sop")=="yes") {
				...
				$sql = "UPDATE ".db_prefix("accounts")." SET clanid=0,clanrank=".CLAN_APPLICANT.",clanjoindate='0001-01-01 00:00:00' WHERE clanid=$dt";
				db_query($sql);
			...
			output("`iLong Name:`i ");
			rawoutput("<input name='clanname' value=\"".htmlentities($claninfo['clanname'], ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\" maxlength=255 size=50>");
			...
			if ($remove>""){
				$sql = "UPDATE " . db_prefix("accounts") . " SET clanrank=".CLAN_APPLICANT.",clanid=0,clanjoindate='0001-01-01 00:00:00' WHERE login='$remove' AND clanrank<={$session['user']['clanrank']}";
				db_query($sql);
