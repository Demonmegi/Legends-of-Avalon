	...
	if ($op=="") {
		...
		if (is_array($history)) {
			if (count($history) == 0) {
				...
				$del = translate_inline("Delete player");
				...
				foreach ($history as $acctid => $acctinfoserialized) {
					...
					if (db_num_rows($res)) {
						...
						if ($session['user']['superuser']&SU_EDIT_USERS) {
							addnav("", "user.php?op=del&userid=$acctid");
							rawoutput("<a href='user.php?op=del&userid=$acctid' onClick=\"return confirm('$conf');\">$del</a>");
						}
						rawoutput("</td><td>");
...
function changename_renamehistory($acctid, $curname)
{
	...
	if (is_array($history) && array_key_exists($acctid, $history)) {
		$userhistory = unserialize($history[$acctid]);
		if ($userhistory['newname'] !=  $curname) {
