<?php
	...
	if( $cityid > 0 )
	{
		...
		$sql .= ( count($field2) > 0 ) ? "citytext = '".mysql_real_escape_string(serialize($field2))."', " : "citytext = '', ";
		$sql .= ( count($field3) > 0 ) ? "stabletext = '".mysql_real_escape_string(serialize($field3))."', " : "stabletext = '', ";
		$sql .= ( count($field4) > 0 ) ? "armortext = '".mysql_real_escape_string(serialize($field4))."', " : "armortext = '', ";
		$sql .= ( count($field5) > 0 ) ? "weaponstext = '".mysql_real_escape_string(serialize($field5))."', " : "weaponstext = '', ";
		$sql .= ( count($field6) > 0 ) ? "mercenarycamptext = '".mysql_real_escape_string(serialize($field6))."', " : "mercenarycamptext = '', ";
		$sql .= ( count($field7) > 0 ) ? "cityblockmods = '".mysql_real_escape_string(serialize($field7))."', " : "cityblockmods = '', ";
		$sql .= ( count($field8) > 0 ) ? "cityblocknavs = '".mysql_real_escape_string(serialize($field8))."' " : "cityblocknavs = '' ";
	...
	else
	{
		...
		while( list($key,$val) = each($post) )
		{
			...
			{
				array_push($cols,"`$key`");
				array_push($vals,mysql_real_escape_string($val));
				unset($post[$key]);
			}
			elseif( in_array($key, $field_array2) )
			{
		...
		if( count($field2) > 0 )
		{
			...
			array_push($vals, mysql_real_escape_string(serialize($field2)));
		}
		if( count($field3) > 0 )
		{
			array_push($cols, 'stabletext');
			array_push($vals, mysql_real_escape_string(serialize($field3)));
		}
		if( count($field4) > 0 )
		{
			array_push($cols, 'armortext');
			array_push($vals, mysql_real_escape_string(serialize($field4)));
		}
		if( count($field5) > 0 )
		{
			array_push($cols, 'weaponstext');
			array_push($vals, mysql_real_escape_string(serialize($field5)));
		}
		if( count($field6) > 0 )
		{
			array_push($cols, 'mercenarycamptext');
			array_push($vals, mysql_real_escape_string(serialize($field6)));
		}
		if( count($field7) > 0 )
		{
			array_push($cols, 'cityblockmods');
			array_push($vals, mysql_real_escape_string(serialize($field7)));
		}
		if( count($field8) > 0 )
		{
			array_push($cols, 'cityblocknavs');
			array_push($vals, mysql_real_escape_string(serialize($field8)));
		}
