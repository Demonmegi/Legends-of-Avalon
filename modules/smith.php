...
function smith_runevent($type, $link)
{
	...
	if ($op=="" || $op=="search"){
		...
		if ($previously_upgraded){
			...
			output("\"`6Aah, I see some butcher has been at this %s`6! I'd never produce such terrible quality.  No matter though, I can easily repair the damage!`7\"`n`n",$session['user'][$op]);
			...
			if ($r <= $ichance){
				...
				output("`7In a few hours, he hands back your %s`0, better than before!", $session['user'][$op]);
				...
				output("He examines it for a moment, then pulls an anvil and forge from behind his back, and sets to work.");
				output("A short while later, he mutters, \"`6Oops.`7\" and hands your %s `0back to you.", $session['user'][$op]);
				output("Your %s `0was downgraded!", $session['user'][$op]);
				$session['user'][$op] = $session['user'][$op]." -1";
				$session['user'][$op.($op=="weapon"?"dmg":"def")]-=1;
				$session['user'][($op=="weapon"?"attack":"defense")]--;
